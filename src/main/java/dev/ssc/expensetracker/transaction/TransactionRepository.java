...
public interface TransactionRepository extends ListCrudRepository<Transaction, Integer> {
      ...
      @Query("select t.id as transaction_id, t.user_id, t.amount, t.type, t.date, a.name as account_name, t.account_id, c.name as category_name, t.category_id from transactions t inner join accounts a on t.account_id = a.id inner join categories c on t.category_id = c.id where t.user_id = :userId")
